...
export const angleDegrees = (a: Vec3, b: Vec3) => {
...
export const projectVehicle = (a: Vec3, gravity: Vec3, forward?: Vec3) => {
  if (!forward) return { longitudinal: norm(a), lateral: 0, magnitudeMode: true };
...
export class GravityCapture {
  ...
  private accelerationEnergy=0;
  add(v: Vec3, acceleration: Vec3 = {x:0,y:0,z:0}) { this.samples.push(v); this.accelerationEnergy += norm(acceleration) ** 2; }
  ...
  accelerationRms() { return this.samples.length ? Math.sqrt(this.accelerationEnergy / this.samples.length) : Infinity; }
